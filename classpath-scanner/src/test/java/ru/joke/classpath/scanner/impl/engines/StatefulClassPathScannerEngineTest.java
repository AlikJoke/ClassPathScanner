...
class StatefulClassPathScannerEngineTest extends AbsClassPathScannerEngineTest {
    ...
    private void executeTest(
        ...
        try (final var mockedStatic = mockStatic(ClassPathResourcesService.class, UUID.randomUUID().toString())) {
            ...
            assertEquals(expectedResources, result, "Resources must be equal");
        }
    }

    @Override
    protected ClassPathScannerEngine createEngine(ClassPathScannerEngineConfiguration.Builder configurationBuilder) {
        return new StatefulClassPathScannerEngine(configurationBuilder.stateful().build());
    }
